...
class SubstituteGenerator:
    ...
    def substitutes_from_probs(
        ...
        probability_list = probs.tolist()
        prob_values = [[probability_list[i][idx] for idx in selected_ids[i]] for i in range(len(selected_ids))]
        ...
        return substitutes, prob_values

    @staticmethod
    def candidates_for_ablation(
            probs: np.ndarray,
            word2id: Dict[str, int],
            candidates: List[List[str]],
            candidate_sent_scores: List[List[float]] = None,
            cadidate_validation_similarities: List[List[float]] = None,
            cadidate_gloss_similarities: List[List[float]] = None,
            s_score: bool = False,
            v_score: bool = False,
            g_score: bool = False,
        ...
        for i in range(probs.shape[0]):
            ...
            org_candidates = candidates[i]

            if s_score:
                sent_scores = [candidate_sent_scores[i][j] for j in range(len(org_candidates)) if
                               org_candidates[j] in candidates_in_vocab]

                _candidate_scores = [
                    1 * sent_scores[id] + 0.05 * candidate_scores[id] for id in
                    range(len(sent_scores))]
                candidate_scores = np.array(_candidate_scores)

            elif v_score:
                val_scores = [cadidate_validation_similarities[i][j] for j in range(len(org_candidates)) if
                              org_candidates[j] in candidates_in_vocab]
                _candidate_scores = [
                    0.05 * candidate_scores[id] + val_scores[id] for id in
                    range(len(val_scores))]
                candidate_scores = np.array(_candidate_scores)

            elif g_score:
                gloss_scores = [cadidate_gloss_similarities[i][j] for j in range(len(org_candidates)) if
                                org_candidates[j] in candidates_in_vocab]
                _candidate_scores = [
                    0.05 * candidate_scores[id] + gloss_scores[id] for id in
                    range(len(gloss_scores))]
                candidate_scores = np.array(_candidate_scores)
    ...
    @staticmethod
    def candidates_from_probs_fitness_gloss(
            probs: np.ndarray,
            word2id: Dict[str, int],
            candidates: List[List[str]],
            candidate_sent_scores: List[List[float]] = None,
        ...
        for i in range(probs.shape[0]):
            ...
            if not (candidate_sent_scores is None):
                org_candidates = candidates[i]
                sent_scores = [candidate_sent_scores[i][j] for j in range(len(org_candidates)) if
                               org_candidates[j] in candidates_in_vocab]
                ...
                candidate_scores = candidate_scores.tolist()
                _candidate_scores = [
                    1 * sent_scores[id] + 0.05 * candidate_scores[id] + val_scores[id] + gloss_scores[id] for id in
                    range(len(sent_scores))]
                candidate_scores = np.array(_candidate_scores)
    ...
    def candidates_for_gap_all(
        ...
        for i in range(probs.shape[0]):
            ...
            if not (candidate_sent_scores is None):
                sent_scores = candidate_sent_scores[i]
                val_scores = cadidate_validation_similarities[i]
                gloss_scores = cadidate_gloss_similarities[i]

                candidate_scores = candidate_scores.tolist()
                _candidate_scores = [(0.05 * candidate_scores[id]) + sent_scores[id] + val_scores[id] + gloss_scores[id]
                                     for id in
                                     range(len(sent_scores))]

            item = []
            for a in range(len(_candidate_scores)):
                item.append([candidates_in_vocab[a], _candidate_scores[a]])

            item.sort(key=lambda x: x[1], reverse=True)
            ranked_candidates_in_vocab.append([i[0] for i in item])

        return ranked_candidates_in_vocab, ranked_candidates

    @staticmethod
    def candidates_for_gap(
            probs: np.ndarray,
            word2id: Dict[str, int],
            candidates: List[List[str]],
            candidate_sent_scores: List[List[float]] = None,
        ...
        ranked_candidates_in_vocab, ranked_candidates = [], []
        for i in range(probs.shape[0]):
            candidates_in_vocab = candidates[i]
            candidate_scores = np.array([
                probs[i, word2id[cand]] for cand in candidates_in_vocab
            ])
            if not (candidate_sent_scores is None):
                ...
                candidate_scores = candidate_scores.tolist()
                _candidate_scores = [1 * sent_scores[id] + 0.05 * candidate_scores[id] for id in
                                     range(len(sent_scores))]
            item = []
            for a in range(len(_candidate_scores)):
                item.append([candidates_in_vocab[a], _candidate_scores[a]])
