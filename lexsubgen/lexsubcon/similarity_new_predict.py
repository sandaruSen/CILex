...
from lexsubgen.lexsubcon.translation import back_translation
from lexsubgen.lexsubcon.reader import Reader_lexical
...
class Similarity_new:
    ...
    def initial_test(self, word, target_word):
        candidate_word = word
        substituability_score = -1
        if candidate_word == target_word or \
                self.lemmatizer.lemmatize(candidate_word, self.pos_initial) == target_word or \
                target_word in candidate_word:
            substituability_score = 0
        return substituability_score
