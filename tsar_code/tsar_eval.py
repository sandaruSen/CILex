...
class TSAR2022_SharedTask_Evaluator(object):
    ...
    def read_files(self, gold_file, labels_file):
        ...
        for line in gold_lines:
            ...
            self.goldinfo[keygold] = {'gold': dict_items.keys(), 'list_values': list_num_counts_items,
                                      'list_keys_lists': list_values_items_lists}
    ...
    def computeAccuracy_at_N_at_top_gold_1(self, N):
        ...
        for key in self.combined.keys():
            ...
            if self.combined[key]['labels'] != None and len(self.combined[key]['labels']) > 0:
                ...
                sentence, complexWord = key.split('__')
                if self.match_group_to_group(self.combined[key]['labels'][0:N],
                                             self.goldinfo[key]['list_keys_lists'][0]):
                    if self.verbose:
    ...
    def computePrecisionMetrics_at_K(self, K):
        ...
        if (MacroAverage_Precision + MacroAverage_Recall) > 0:
            MacroAverage_F1 = 2 * MacroAverage_Precision * MacroAverage_Recall / (
                    MacroAverage_Precision + MacroAverage_Recall)
            MicroAverage_F1 = 2 * MicroAverage_Precision * MicroAverage_Recall / (
                    MicroAverage_Precision + MicroAverage_Recall)
...
def main():
    ...
    parser.add_option('--gold_file', metavar='<PATH>', action='store', type='string', dest='gold_file', default='tsar2022_en_test_gold.tsv',
                      help='The path to the file with the gold annotated instances')
    parser.add_option('--predictions_file', metavar='<PATH>', action='store', type='string', dest='predictions_file',
                      default='tsar2022_test_en_CILS_4.tsv', help='The path to file with the predictions')
    parser.add_option('--output_file', metavar='<PATH>', action='store', type='string', dest='output_file', default='cils4.tsv.eval',
    ...
    MacroAverage_Precision_at_1, MacroAverage_Recall_at_1, MacroAverage_F1_at_1, MicroAverage_Precision_at_1, MicroAverage_Recall_at_1, MicroAverage_F1_at_1, Potential_at_1 = evaluator.computePrecisionMetrics_at_K(
        1)
    MacroAverage_Precision_at_3, MacroAverage_Recall_at_3, MacroAverage_F1_at_3, MicroAverage_Precision_at_3, MicroAverage_Recall_at_3, MicroAverage_F1_at_3, Potential_at_3 = evaluator.computePrecisionMetrics_at_K(
        3)
    MacroAverage_Precision_at_5, MacroAverage_Recall_at_5, MacroAverage_F1_at_5, MicroAverage_Precision_at_5, MicroAverage_Recall_at_5, MicroAverage_F1_at_5, Potential_at_5 = evaluator.computePrecisionMetrics_at_K(
        5)
    MacroAverage_Precision_at_10, MacroAverage_Recall_at_10, MacroAverage_F1_at_10, MicroAverage_Precision_at_10, MicroAverage_Recall_at_10, MicroAverage_F1_at_10, Potential_at_10 = evaluator.computePrecisionMetrics_at_K(
        10)
...
# python3 tsar_eval.py --gold_file tsar2022_en_trial_gold.tsv --predictions_file test.tsv  --output_file CILS.tsv.eval